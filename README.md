...
- Quantum-switch “Jones microscope” tools (`make_ico_main_figure.py`, `generate_ico_curves.py`) for the commutator-contrast and witness studies in Sec. V.
- Paper assets (`quantum_eraser_imaging.pdf`, `intro_fig.png`) and demo outputs illustrating how each script connects back to the manuscript.
...
## Repository Overview
| File | Role | Paper tie-in |
| --- | --- | --- |
| `quantum_eraser_imaging.pdf` | Full manuscript | Reference for all figures/equations |
| `qei_monte_carlo_gpu.py` | Monte-Carlo simulator for absorption/phase QEI and optional ICO module | Secs. II–IV (Figs. 2–4), Sec. V (Eq. 27) |
| `make_ico_main_figure.py` | Quantum-switch scene builder + commutator contrast figure | Sec. V main text (Fig. 6) |
| `generate_ico_curves.py` | Parameter sweeps for ICO tuning curves | Sec. V supplemental discussion |
| `intro_fig.png` | Interferometer schematic from the paper | Fig. 1 |
| `butterfly.jpg` | Demo amplitude texture used in the example figures | All demo commands |
| `butterfly_*png` | Pre-generated outputs shown below | Generated by commands in Featured Outputs |
...
| `python qei_monte_carlo_gpu.py --H 256 --W 256 --ppp 800 --image butterfly.jpg --phi-mode smooth --nofig --save --save-prefix butterfly_qei` | `python qei_monte_carlo_gpu.py --H 256 --W 256 --ppp 800 --image butterfly.jpg --phi-mode smooth --ico --nofig --save --save-prefix butterfly_qei_ico` |

| ICO tuning curves (`butterfly_ico_curves.png`) |
| :--: |
| ![ICO tuning curves](butterfly_ico_curves.png) |
| `python generate_ico_curves.py --H 128 --W 128 --ppp 1000 --image butterfly.jpg --phi-mode smooth --out butterfly_ico_curves.png` |
